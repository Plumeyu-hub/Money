...
public class BudgetActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 第二步:初始化数据
		blist = new ArrayList<BudgetData>();
		// 实例化适配器
		...
		blv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				BudgetData bData = blist.get(position);
			}
		});
		// 长按删除
	...
	public void refresh() {
		...
		if (cs != null) {
			...
			while (cs.moveToNext()) {
				...
				bData = new BudgetData(bmoney, bremarks, btime, bid, buserid,
						cid);
				blist.add(bData);
			}
		}
		Collections.sort(blist, new Comparator<BudgetData>() {
			@Override
			public int compare(BudgetData lhs, BudgetData rhs) {
				// TODO Auto-generated method stub
